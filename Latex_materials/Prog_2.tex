...
\begin{document}
	...
	By definition these schemes must always give identical results if contributions from all orders are added, however the manner of ordering the pertubative expansion is not consistent between the two schemes at any given order. This allows the possibility of a difference between the two schemes to arise, however this can only occur when the schemes are considered at a fixed order. There are advantages and disadvantages to each of the schemes \cite{Article:FlavourSchemes}, and this analysis allows an insight into both the 4FNS and 5FNS. \par
...
An example of this process can be seen by considering the constraint placed on the SM by electroweak (EW) theory: under EW theory, the masses of the W$^{\pm}$ and Z$^{0}$ bosons (i.e. the gauge bosons of the weak force) violate invariance under local gauge transformations. However, despite this the W$^{\pm}$ and Z$^{0}$ bosons have large masses. In order to account for this within the SM, the Higgs mechanism is required. The Higgs mechanism results in the presence of an additional gauge boson with a mass of 125 GeV in the SM known as the Higgs boson \cite{DiscoHiggsATLAS}\cite{DiscoHiggsCMS}. The Higgs mechanism is described in greater detail in section 2.2.3. \par
		...
		The potential is dependent upon the choice of $\mu$ and $\lambda$, as this selection constrains the bounds of the potential. In the instance where $\mu^{2} < 0$ and $\lambda> 0$ the potential is bounded from below, and has a rotationally symetric degenerate ground state
		...
		$\phi(x)$ is expanded using Eq. 12 via pertubation theory. The ground state can be fixed to $\phi_{1}=\phi_{2}=\phi_{4} = 0$ and $\phi_{3} = v$ at
		...
		\begin{figure}[h!]
			...
			\begin{subfigure}{.5\textwidth}
				...
				\caption{The final state produced by interactions of a gluon pair.}
		...
		\begin{figure} [h!]
			\begin{subfigure}{.5\textwidth}
				\centering
				\includegraphics[scale=0.35]{Gluon_ttbar}
				\caption{The Top Quark Pair Production background process.}
				\label{Fig:SFigTtbarFeynman}
			\end{subfigure}
			\begin{subfigure}{.5\textwidth}
				\centering
				\includegraphics[scale=0.45]{Single_Quark}
		...
		\begin{figure}[h]
			\begin{subfigure}{.5\textwidth}
				\centering
				\includegraphics[scale=0.475]{ZqqZll}
				\caption{The ZqqZll background process.}
				\label{Fig:SFigZqqZllFeynman}
			\end{subfigure}
			\begin{subfigure}{.5\textwidth}
				\centering
				\includegraphics[scale=0.45]{WqqZll}
	...
	\section{LHC and ATLAS}
		...
		100 meters beneath the site of the Conseil Européen pour la Recherche Nucléaire (CERN) at the French-Swiss border near Geneva, Switzerland lies the LHC. At approximately 27 kilometers in circumference, the LHC accelerates beams of protons to the highest energies in the world 
		...
		In order to evaluate the performance of an accelerator, it is important to consider a metric that adequately describes the collisions taking place. In the case of accelerator physics "luminosity", a measure of how effectively an accelerator is able to deliver particle collisions, is used. The luminosity of an accelerator is defined as the integral of the accelerator's instantaneous luminosity with respect to time, i.e. the instantaneous luminosity at every instant or:
		...
		\begin{equation}
			\mathcal{L} = \frac{N_{1}N_{2}kf\gamma}{4\pi\epsilon_{n}\beta^{*}}F
		\end{equation}
		...
		Integrated luminosity has units of cm$^{-2}$ but is more commonly expressed in units of inverse femtobarn fb$^{-1}$ where one barn (1 b) is equivalent to $1 \times 10^{-28} $m$^{-2}$. During Run 2 the ATLAS detector recorded a total of 140fb$^{-1}$ of luminosity corresponding to 3.2fb$^{-1}$ in 2015, 32.9fb$^{-1}$ in 2016, 43.7fb$^{-1}$ in 2017, and 60.1fb$^{-1}$ in 2018 \cite{ATLASLumiPublic}. 

		\begin{figure}[h!]
			\begin{subfigure}{.5\textwidth}
				\centering
				\includegraphics[scale=0.3]{ATLAS_Luminosity_Time.png}
				\caption{Delivered luminosity vs. time}
				\label{Fig:SFigATLASLuminosityTime}
			\end{subfigure}
			\begin{subfigure}{.5\textwidth}
				\centering
				\includegraphics[scale=0.3]{ATLAS_Luminosity_Total.png}
		...
		\newpage
		The Large Hadron Collider (LHC) \cite{Article:LHCDesignPaper} is a 26.7km particle accelerator and collider. During its second run of data collection (appropriately named Run 2) the LHC collides particle bunches of up to $10^{11}$ particles at a centre of mass energy $\sqrt{s}$ = 13 Tev 40 million times each second in order to achieve a design luminosity of $10^{34}$cm$^{-2}$s$^{-1}$. The protons are sourced from hydrogen atoms in a bottle of gas - the hydrogen is passed through an electric field to strip away its electrons, and the resulting protons are then accelerated through a series of smaller accelerators to increase their energy to desired levels. \par
		
		The protons are accelerated to 50 MeV by LINAC 2, up to 1.4 GeV by the Proton Synchrotron Booster (PSB), then to 25 GeV by the Proton Synchrotron (PS). The final step before the protons are injected into the main LHC pipes is an acceleration to 450 GeV by the Super Proton Synchrotron (SPS). The protons are injected evenly into each of the two LHC beam pipes where they are accelerated in opposite directions to their maximum energy of 6.5 TeV \cite{Article:LHCDesignPaper}.
		Each of these accelerators can be seen in Figure \ref{Fig:CERNRings}. \par
		
		Once the protons have reached their maximum energy, they are focussed and collided at one of the four interaction points around the circumference of the LHC ring. The collisions that produce the data used in this analysis occur at Point 1: The ATLAS detector \cite{Article:LHCDesignPaper}. \par 
		
		\begin{figure}
			\centering
			\includegraphics[scale=0.5]{LHC_Rings}
			\caption{A figure showing the rings of the CERN accelerator complex \cite{Article:CernComplex}.}
			\label{Fig:CERNRings}
		\end{figure}
...
`		\end{itemize}
		...
		\bf \underline{February 2020 - April 2020: Production of top and multijet enriched control regions}
