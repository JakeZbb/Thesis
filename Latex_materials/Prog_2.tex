...
\begin{document}
	...
	\subsubsection{Standard Model Particle Content}
	...
	Colour in this sense does not refer to the typical description of colour, but a quantum number that parametrises these strong interactions with the value of either red ($r$), green ($g$), or blue ($b$) for particles and anti-red ($\bar{r}$), anti-green ($\bar{g}$) or anti-blue ($\bar{b}$) for anti-particles. The strong force binds particles with complementary colour charges together in order to create colourless states with no overall colour: either an equal mixture of red, green, and blue ($rgb$), an equal mixture of anti-red, anti-green, and anti-blue ($\bar{r}\bar{g}\bar{b}$), or a colour-anticolour pair ($r\bar{r}$,$g\bar{g}$,$b\bar{b}$). This powerful binding by the strong force prevents the existence of individual quarks, causing quarks to be bound into colourless states called hadrons by the phenomenon of colour confinement. Hadrons consisting of three quarks/anti-quarks are called baryons/anti-baryons, and hadrons consisting of quark-antiquark pairs are called mesons. \par

	Each generation of quark contains a pair of one up-type and one down-type quark with contrasting charge ($Q$) and unique quantum numbers. These generations in order of increasing mass are up ($u$) and down ($d$), charm ($c$) and strange ($s$), and top ($t$) and bottom ($b$). The quantum numbers for each of the quarks are:

	\begin{itemize}
		\item Charge ($Q$): Up-type quarks have charge $+\frac{2}{3}$, down-type quarks have charge $-\frac{1}{3}$
		\item Isospin ($I_{3}$): The first generation of quarks have assigned third component isospins which is used to distinguish hadrons of similar mass. 
		\item Charmness ($C$)/Topness ($T$): A flavour number for the heavier generations of up-type quarks. This value is 1 for its respective quark, and 0 for all other quarks.
		\item Strangeness ($S$)/Bottomness ($B$): A flavour number for the heavier generations of down-type quarks. This value is -1 for its respective quark, and 0 for all other quarks.   
	\end{itemize}

	A list of quarks with their fully detailed quantum numbers can be seen in Table \ref{tab:SMQuarks1}.

	\begin{table}
		\begin{center}
			\begin{tabular}{ c|c|c|c|c|c|c|c }
				\hline \hline
				Quark Flavour & Mass [MeV] & $Q$ (e) & $I_{3}$ & $C$ & $S$ & $T$ & $B$ \rule{0pt}{2.6ex}\rule[-0.9ex]{0pt}{0pt}\\ 
				\hline \hline
				u & 2.2 & $+\frac{2}{3}$ & $+\frac{1}{2}$ & 0 & 0 & 0 & 0 \rule{0pt}{2.6ex}\rule[-0.9ex]{0pt}{0pt}\\ 
				d & 4.7 & $-\frac{1}{3}$ & $-\frac{1}{2}$ & 0 & 0 & 0 & 0 \rule{0pt}{2.6ex}\rule[-1.6ex]{0pt}{0pt}\\ 
				\hline
				c & 1.28$\times 10^{3}$  & $+\frac{2}{3}$ & 0 & 1 & 0 & 0 & 0 \rule{0pt}{2.6ex}\rule[-0.9ex]{0pt}{0pt}\\ 
				s & 95 & $-\frac{1}{3}$ & 0 & 0 & -1 & 0 & 0 \rule{0pt}{2.6ex}\rule[-1.6ex]{0pt}{0pt}\\
				\hline
				t & 173$\times 10^{3}$ & $+\frac{2}{3}$ & 0 & 0 & 0 & 1 & 0 \rule{0pt}{2.6ex}\rule[-0.9ex]{0pt}{0pt}\\
				b & 4.18$\times 10^{3}$ & $-\frac{1}{3}$ & 0& 0 & 0 & 0 & -1 \rule{0pt}{2.6ex}\rule[-1.6ex]{0pt}{0pt}\\
				\hline \hline
			\end{tabular}
				\caption{A table showing the mass, charge, and quantum numbers of the quarks described by the SM. The first, second, and third rows correspond to the first, second, and third quark generations respectively \cite{Article:PDG}.}
				\label{tab:SMQuarks1}
		\end{center}
	\end{table}
	...
	\subsubsection{The SM Lagrangian}
